...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="fr" lang="en">
    <head>
        ...
        <script type="text/javascript" src="/media/js/ajaxupload.js"></script>
    ...
    <body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
        ...
        <div id="page" class="{{user.type|slug_title|safe}}_border">
            <div id="sub_header" class="{{user.type|slug_title|safe}}">
                <span class="ariane">{% block ariane %}<a href="/user/page/{{user.id}}">{% if invite_payeur %}<b>{% endif %}{{user.name}} date d'inscription {{user.registration_date}}{% if invite_payeur %}</b>{% endif %}</a>{% endblock %}</span>
